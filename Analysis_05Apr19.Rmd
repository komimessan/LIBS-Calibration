...
lead.plsr <- plsr(pb_concentration ~ pb_mean_intensity, ncomp = 3, data = Lead_mval_final, validation = "LOO")

summary(lead.plsr)

lead.par <- par(mfrow=c(1,2))
pb_rmsep <- plot(RMSEP(lead.plsr), legendpos = "topright") #Root Mean Square Error of the Prediction
pb_predict_meas <- plot(lead.plsr, ncomp = 2, asp = 1, line = TRUE) # predicted vs. measured plot
par(lead.par)
...
zn_rmsep <- plot(RMSEP(zinc.plsr), legendpos = "topright") #Root Mean Square Error of the Prediction
zn_predict_meas <- plot(zinc.plsr, ncomp = 2, asp = 1, line = TRUE) # predicted vs. measured plot
par(zinc.par)
...
sb_rmsep <- plot(RMSEP(antimony.plsr), legendpos = "topright") #Root Mean Square Error of the Prediction
sb_predict_meas <- plot(antimony.plsr, ncomp = 3, asp = 1, line = TRUE) # predicted vs. measured plot
par(antimony.par)
...
w_rmsep <- plot(RMSEP(tungsten.plsr), legendpos = "topright") #Root Mean Square Error of the Prediction
w_predict_meas <- plot(tungsten.plsr, ncomp = 2, asp = 1, line = TRUE) # predicted vs. measured plot
par(tungsten.par)
