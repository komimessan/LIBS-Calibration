...
The average of each of the nine shots were taken by only considering the positive values using the following function and also function to calculate all Rsquare values
...
for (i in 1:L){
  linear_relation <- lm(as.numeric(Tungsten_Data_mval[i,2:6])~concentration)
...
l_w <-230 # lowest value of wavelength to display
...
## Calibration curve for zinc at 492.40132 nm

From the access to the National Institute of Standards and Technology (NIST) atomic emission spectrum database, the higher intensity of zinc (Zn) occurs at 492.40132 which is approximately equal to 492.4 thus we select 492.4 for our univariate analysis. 
...
# R_zn[which(R_zn$Rsquare>0.89 & R_zn$Rsquare<0.92 & R_zn$Wavelength<500),]
...
cali_plot_zn <- ggplot(zinc_uni_data, aes(x=Concentration, y=Mean_I)) +
  ...
  ylab("Intensity (a.u.)") +
  geom_label(x=10, y=900, label="R = 0.3459") + 
  theme_bw() + p
...
## Calibration curve for antimony at 334.50134 nm

From the access to the National Institute of Standards and Technology (NIST) atomic emission spectrum database, the higher intensity of Antimony (Sb) occurs at 334.50134 which is approximately equal to 334.5 thus we select 334.534 for our univariate analysis. 
...
cali_plot_sb <- ggplot(antimony_uni_data, aes(x=Concentration, y=Mean_I)) +
  ...
  ylab("Intensity (a.u.)") +
  geom_label(x=10, y=75, label="R = 0.8994") + 
  theme_bw() + p
...
## Calibration curve for tungsten at 334.50134 nm
...
cali_plot_w <- ggplot(tungsten_uni_data, aes(x=Concentration, y=Mean_I)) +
  ...
  ylab("Intensity (a.u.)") +
  geom_label(x=0, y=75, label="R = 0.8043") + 
  theme_bw() + p
